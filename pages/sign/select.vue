<template>
	<view class="select-location">
		...
		<view class="top-search">
			<van-search :value="keyword" background="rgba(255, 255, 255, 0)" shape="round" readonly placeholder="关键词搜索"
			 @click.native="goSearch">
			</van-search>
		</view>
		<map id="map" :latitude="latitude" :longitude="longitude" @regionchange="changeEvent">
			<!-- 定位标志 -->
			<cover-image class="location-icon" src="/static/images/location.png">
			</cover-image>
			<!-- tip -->
		...
		<view class="search-area">
			<view class="search-list">
				<scroll-view class="list" scroll-y>
					<view :class="['list-item', 'flex', 'van-hairline--bottom', active === item.id ? 'active' : '']" v-for="(item) in list"
					 :key="item.id" @tap="selectedAddress(item)">
						<van-icon v-if="active === item.id" class="icon" name="checked" size="34rpx" :color="active === item.id ? '#007aff' : ''">
						</van-icon>
						<view class="info">
...
<script>
	...
	export default {
		...
		methods: {
			...
			goSearch() {
				uni.navigateTo({
					url: `/pages/sign/search?longitude=${this.longitude}&latitude=${this.latitude}`
				})
			},
