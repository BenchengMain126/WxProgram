	...
	<div class="signin">
		<view class="location">
			<van-cell class="field-cell" required title-width="70px" title="当前定位：">
				...
				<van-button v-if="location.curLocation" slot="right-icon" type="primary" size="mini" @click="handleSelect">
					位置微调
				</van-button>
			</van-cell>
		</view>
		<view class="signin-area">
			<van-button class="signin-btn" type="primary" round :disabled="signin.isSignin" @click="handleSignin">
		...
		methods: {
			...
			handleSelect() {
				if (this.location.curLocation) {
					...
					uni.navigateTo({
						url: `/pages/sign/select?longitude=${lng}&latitude=${lat}`
					})
				} else {
					this.getLocation()
				}
